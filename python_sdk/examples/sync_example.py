...
def main():
    parser = argparse.ArgumentParser(description="Calls the Headless BI query_datasource and read_metadata APIs synchronously")
    ...
    with server.auth.sign_in(tableau_auth):
        ...
        sample_superstore_luid=[ds.id for ds in all_datasources if ds.name == 'Superstore Datasource'][0]
        print(f'Using {sample_datasource.name} with id {sample_superstore_luid}')

        # sample query_datasource
        query_request_json = file_util.read_json('payloads','query_request.json')
        query_request = QueryRequest.from_json(query_request_json)
        query_request.datasource.datasource_luid = sample_superstore_luid
        response = client.query_datasource(args.server + EndPoints.VIZQL_DATA_SERVICE_URL, headers.to_dict(),
                                        query_request)
        if response:
        ...
        readmetadata_request_json = file_util.read_json('payloads','read_metadata_request.json')
        ...
        response=client.read_metadata(args.server + EndPoints.VIZQL_DATA_SERVICE_URL, headers.to_dict(), read_metadata_request)
        if response:
