...
async def main():
    ...
    with server.auth.sign_in(tableau_auth):
        ...
        print(f'Using {sample_datasource.name} in the  with id {sample_superstore_luid}')

        # sample query_datasource
        query_request = QueryRequest.from_json(file_util.read_json('payloads','query_request.json'))
        query_request.datasource.datasource_luid = sample_superstore_luid
        response = await client.query_datasource(args.server + "".join([EndPoints.VIZQL_DATA_SERVICE_URL,
        ...
        response= await client.read_metadata(args.server + "".join([EndPoints.VIZQL_DATA_SERVICE_URL,
